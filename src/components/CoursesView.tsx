              ...
              {subscribeSuccessMessage ? (
                ...
                <div className="space-y-3 pt-2">
                  <button
                    ...
                    className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-slate-950 font-black text-sm shadow-md transition flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
                  >
                    {isSubscribing ? (
                      <span className="flex items-center gap-2">جاري تفعيل الاشتراك السنوي...</span>
                    ) : (
                      <>
                        <Sparkles className="w-4 h-4 fill-current" />
                        <span>تأكيد الاشتراك وتفعيل الباقة السنوية</span>
                      </>
                    )}
                  </button>

                  <p className="text-[10px] text-slate-400">
                    * الاشتراك فوري وسارٍ لمدة 365 يوماً من تاريخ التفعيل.
                  </p>
                </div>
              )}
            </div>
